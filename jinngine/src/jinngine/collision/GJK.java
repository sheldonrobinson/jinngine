...
/**
 * Implementation of the Gilbert-Johnsson-Keerthi algorithm, for computing the distance between convex objects and/or
 * closest points. This implementation is based on the theory presented in [Bergen, 2003, Continuous Collision Detection
 * in Interactive 3D environments, ISBN-13: 978-1558608016]. This describes an approach that is based on a recursive
 * simplex reduction technique. This implementation us "unrolled", so that recursive calls are avoided.
 */
public final class GJK {
    /**
     * A state structure for the GJK3 implementation. Used internally in the GJK3 implementation, and also used to
     * simplify caching of GJK results.
     */
     ...
     * method does not take into account the value of {@link SupportMap3.sphereSweepRadius()}. This is so, because the
     * closest points of two sphere swept volumes are trivially known if the closest points of the original shapes are
     * known.
     * 
     * @param Sa
     *            support mapping of a convex object A
     * @param Sb
     *            support mapping of a convex object B
     * @param va
     *            on return, va will contain the closest point on A
     * @param vb
     *            on return, vb will contain the closest point on B
     * 
     * @param envelope
     *            the maximum distance that GJK should compute. If the algorithm reaches a state where the true distance
     *            between A and B will be greater the given value, it terminates. Setting this value Inf will result in
     *            the algorithm computing the distance between A and B in all cases. Setting it to zero will reduce the
     *            algorithm to computing a separating axis, because it will terminate as soon as evidence that A and B
     *            are non-intersecting is found.
     * @param epsilon
     *            error precision, used in termination criterion. In general, a smaller error tolerance results in more
     *            iterations before termination.
     * 
     * @param maxiter
     *            maximum number of iterations before termination is forced.
     */
    public void run(final SupportMap3 Sa, final SupportMap3 Sb, final Vector3 va, final Vector3 vb,
            final double envelope, final double epsilon, final int maxiter) {
        ...
        while (true) {
            ...
            if (Math.abs(v.dot(v) - v.dot(w)) < epsilon * epsilon || state.iterations > maxiter
                    || state.simplexSize > 3) {
                break;
            }

            // add w to the simplices
            final Vector3[] row = state.simplices[state.permutation[state.simplexSize]];
            row[0].assign(w);
            row[1].assign(sa);
            row[2].assign(sb);
            row[3].assign(v);
            ...
            for (int i = 0; i < state.simplexSize; i++) {
                // Vector3.add(v, state.simplices[state.permutation[i]][0].multiply(
                // state.lambda[state.permutation[i]]));
        ...
        for (int i = 0; i < state.simplexSize; i++) {
            // Vector3.add(state.p,
            // state.simplices[state.permutation[i]][1].multiply(state.lambda[state.permutation[i]]));
            // Vector3.add(state.q,
            // state.simplices[state.permutation[i]][2].multiply(state.lambda[state.permutation[i]]));
            Vector3.multiplyAndAdd(state.simplices[state.permutation[i]][1], state.lambda[state.permutation[i]],
                    state.p);
            Vector3.multiplyAndAdd(state.simplices[state.permutation[i]][2], state.lambda[state.permutation[i]],
                    state.q);
        }
    ...
    /**
     * updateSimplex() uses the cached information in GJK3.State to update the current simplex. More specifically, the
     * cached v vectors, that was previously used to obtain each simplex point, is reused to recompute all simplex
     * points. After this, the method reduceSimplex is invoked, such that the new updated simplex is indeed a minimal
     * one. The relevance in this lies in exploiting frame coherence. When GJK is run on a given configuration, the v
     * vectors used to obtain the current simplex will often be sufficient to obtain a new simplex, adequate to meet the
     * termination condition for the next time frame. For example, this is would often be the case for resting contacts.
     * Thus all subsequent GJK iterations is avoided.
     */
    private final void updateSimplex(final GJK.State state, final SupportMap3 Sa, final SupportMap3 Sb) {
        ...
        state.v.assignZero();
        for (int i = 0; i < state.simplexSize; i++) {
            // Vector3.add(state.v, state.simplices[state.permutation[i]][0].multiply(
            // state.lambda[state.permutation[i]]));
            Vector3.multiplyAndAdd(state.simplices[state.permutation[i]][0], state.lambda[state.permutation[i]],
                    state.v);
        }
    ...
    /**
     * Method is a hard-coded method that reduces the given simplex to the smallest possible simplex, that contains the
     * closest point to the origin. This approach is described in detail in [Bergen 2003], and can be quite cumbersome
     * to grasp. However, it is sufficient to know that the method finds the smallest subset of simplex points, which
     * contains the closest point to the origin of the whole simplex. In addition, the method also computes a
     * parametrisation of the closest point, v, expressed as a convex combination of the new simplex points, given by
     * the lambda coefficients.
     * 
     * @param state
     *            State containing the simplex that is to be reduced
    ...
    private final boolean reduceSimplex(final State state) {
        ...
        switch (state.simplexSize) {
            ...
            case 3: {
                ...
                // y1,y2 (no permutation)
                // final double d123_3 = d12_1 * y1.sub(y3).dot(y1) + d12_2 * y1.sub(y3).dot(y2); //d123_3 =
                // Math.abs(d123_3)<epsilon?0:d123_3;
                final double d123_3 = d12_1 * Vector3.subAndDot(y1, y3, y1) + d12_2 * Vector3.subAndDot(y1, y3, y2); // d123_3
                                                                                                                     // =
                                                                                                                     // Math.abs(d123_3)<epsilon?0:d123_3;
                ...
                if (d123_1 > epsilon && d123_2 > epsilon && d123_3 > epsilon) {
                    ...
                    // double d123_3 = d12_1 * y1.minus(y3).dot(y1) + d12_2 * y1.minus(y3).dot(y2); d123_3 =
                    // Math.abs(d123_3)<epsilon?0:d123_3;
                    lambda[perm[0]] = d12_1 / d12;
                    lambda[perm[1]] = d12_2 / d12;
                    state.simplexSize = 2;
            ...
            case 4: {
                ...
                // y1,y2,y3 (no permutation)
                // final double d1234_4 = d123_1 * (y1.sub(y4).dot(y1)) + d123_2 * (y1.sub(y4).dot(y2) ) + d123_3 *
                // (y1.sub(y4).dot(y3));
                final double d1234_4 = d123_1 * Vector3.subAndDot(y1, y4, y1) + d123_2 * Vector3.subAndDot(y1, y4, y2)
                        + d123_3 * Vector3.subAndDot(y1, y4, y3);
                ...
                // y1,y2,y4 (4,3)
                // final double d1234_3 = d124_1 * (y1.sub(y3).dot(y1)) + d124_2 * (y1.sub(y3).dot(y2) ) + d124_4 *
                // (y1.sub(y3).dot(y4));
                final double d1234_3 = d124_1 * Vector3.subAndDot(y1, y3, y1) + d124_2 * Vector3.subAndDot(y1, y3, y2)
                        + d124_4 * Vector3.subAndDot(y1, y3, y4);
                ...
                // y1,y3,y4 (3,2) (4,3)
                // final double d1234_2 = d134_1 * (y1.sub(y2).dot(y1)) + d134_3 * (y1.sub(y2).dot(y3) ) + d134_4 *
                // (y1.sub(y2).dot(y4));
                final double d1234_2 = d134_1 * Vector3.subAndDot(y1, y2, y1) + d134_3 * Vector3.subAndDot(y1, y2, y3)
                        + d134_4 * Vector3.subAndDot(y1, y2, y4);
                ...
                // final double d1234_1 = d234_2 * (y2.sub(y1).dot(y2)) + d234_3 * (y2.sub(y1).dot(y3) ) + d234_4 *
                // (y2.sub(y1).dot(y4));
                final double d1234_1 = d234_2 * Vector3.subAndDot(y2, y1, y2) + d234_3 * Vector3.subAndDot(y2, y1, y3)
                        + d234_4 * Vector3.subAndDot(y2, y1, y4);
                ...
                if (d1234_1 > epsilon && d1234_2 > epsilon && d1234_3 > epsilon && d1234_4 > epsilon) {
                    ...
                    // if(
                    // y1.multiply(d1234_1/d1234).add(y2.multiply(d1234_2/d1234)).add(y3.multiply(d1234_3/d1234)).add(y4.multiply(d1234_4/d1234)).norm()
                    // > 0.001 ) {
                    //
                    // System.out.println("d1234="+d1234+"  wrong penetration: "+
                    // y1.multiply(d1234_1/d1234).add(y2.multiply(d1234_2/d1234)).add(y3.multiply(d1234_3/d1234)).add(y4.multiply(d1234_4/d1234)).norm());
                    // state.simplexSize = 3;
                    // modified = false;
                    // break;
