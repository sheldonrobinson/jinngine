...
public final class RayCast {
     ...
     * 
     * @param Sb
     *            support mapping of a convex shape
     * @param Sc
     *            optional support mapping. If this support map is given, it is "added" to the ray. The ray cast then
     *            becomes a convex cast, where the convex shape given by Sc, is casted against Sb, starting at the given
     *            point and along the ray direction
     * @param point
     *            point on ray
     * @param direction
     *            direction of ray
     * @param pb
     *            upon termination, pb will contain the closest point on Sb
     * @param pc
     *            upon termination, pc will contain the closest point on Sc. If Sc is not given, the pc will contain the
     *            end-point of the ray
     * @param lambda
     *            starting ray parameter. Defaults to zero
     * @param envelope
     *            they ray is defined to hit the object if it is within this distance
     * @param epsilon
     *            the desired accuracy (directly passed to GJK)
     * @param sweep
     *            when true, the ray-cast will take sphere sweeping into account.
     ...
     */
    public final double run(final SupportMap3 Sb, final SupportMap3 Sc, final Vector3 point, final Vector3 direction,
            final Vector3 pb, final Vector3 pc, double lambda, final double envelope, final double epsilon,
            final boolean sweep) {
        ...
        if (Sc == null) {
            Sa = new SupportMap3() {
                @Override
                public final Vector3 supportPoint(final Vector3 direction, final Vector3 result) {
                    return result.assign(new Vector3(x));
                }

                @Override
                public final void supportFeature(final Vector3 d, final Iterator<Vector3> returnList) {}
            ...
            Sa = new SupportMap3() {
                @Override
                public final Vector3 supportPoint(final Vector3 direction, final Vector3 result) {
                    return result.assign(x.add(Sc.supportPoint(direction, new Vector3())));
                }
        ...
        while (true) {
            ...
            if (v.dot(direction) >= 0) {
                ...
                final Vector3 vs = v.sub(v.normalize().multiply(envelope * 0.5 + sphere));
