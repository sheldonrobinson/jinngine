...
//	}
	...
	private final Vector3 getMaxBoundsTmp2(Vector3 bounds) {
		...
		bounds.assign( 
				xs*(T.a11<0?-0.5:0.5)*T.a11+ys*(T.a12<0?-0.5:0.5)*T.a12+zs*(T.a13<0?-0.5:0.5)*T.a13,
				xs*(T.a21<0?-0.5:0.5)*T.a21+ys*(T.a22<0?-0.5:0.5)*T.a22+zs*(T.a23<0?-0.5:0.5)*T.a23,
				xs*(T.a31<0?-0.5:0.5)*T.a31+ys*(T.a32<0?-0.5:0.5)*T.a32+zs*(T.a33<0?-0.5:0.5)*T.a33 );
...
//	}
	...
	private final Vector3 getMinBoundsTmp2(Vector3 bounds) {
		final Matrix4 T = worldTransform;
	...
	public final void supportFeature(final Vector3 direction, final Iterator<Vector3> list) {
		...
		final Vector3 p2 = list.next();
		 ...
		 if ( Math.abs(v.y) < epsilon && Math.abs(v.z) < epsilon ) {
			 ...
			 if (v.y >= 0) {
				 p1.z =  0.5; p1.x =  0.5; p1.y = .5;
				 p2.z = -0.5; p2.x =  0.5; p2.y = .5;
				 p3.z = -0.5; p3.x = -0.5; p3.y = .5;
				 p4.z =  0.5; p4.x = -0.5; p4.y = .5;
			 } else {
				 p1.z =  0.5; p1.x =  0.5; p1.y = -.5;
				 p2.z =  0.5; p2.x = -0.5; p2.y = -.5;
				 p3.z = -0.5; p3.x = -0.5; p3.y = -.5;
			 ...
			 if (v.z >= 0) {
				 p1.x =  0.5; p1.y =  0.5; p1.z = .5;
				 p2.x = -0.5; p2.y =  0.5; p2.z = .5;
				 p3.x = -0.5; p3.y = -0.5; p3.z = .5;
				 p4.x =  0.5; p4.y = -0.5; p4.z = .5;
			 } else {
				 p1.x =  0.5; p1.y =  0.5; p1.z = -.5;
				 p2.x =  0.5; p2.y = -0.5; p2.z = -.5;
				 p3.x = -0.5; p3.y = -0.5; p3.z = -.5;
