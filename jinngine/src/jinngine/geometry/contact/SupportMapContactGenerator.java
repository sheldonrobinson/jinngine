...
public final class SupportMapContactGenerator implements ContactGenerator {
    ...
    private final Pool<Vector3> faceA = new Pool<Vector3>(new ArrayList<Vector3>(), new Pool.Factory<Vector3>() {
        @Override
    ...
    private final Pool<Vector3> faceB = new Pool<Vector3>(new ArrayList<Vector3>(), new Pool.Factory<Vector3>() {
        @Override
        public final Vector3 getNewInstance() {
            return new Vector3();
        }
    ...
    private final Pool<Vector3> intersection = new Pool<Vector3>(new ArrayList<Vector3>(), new Pool.Factory<Vector3>() {
        @Override
        public final Vector3 getNewInstance() {
            return new Vector3();
        }
    ...
    public SupportMapContactGenerator(final SupportMap3 sa, final Geometry ga, final SupportMap3 sb, final Geometry gb) {
        Sa = sa;
        Sb = sb;
        this.ga = ga;
        this.gb = gb;
        ...
        spa = Sa.sphereSweepRadius();
        spb = Sb.sphereSweepRadius();
    ...
    public void run(final Result result) {
        ...
        if (gjk.getState().intersection) {
            ...
            final Vector3 direction = ga.getBody().getPosition().add(gadisp)
                    .sub(gb.getBody().getPosition().add(gbdisp));
            ...
            final Vector3 sp = Sa.supportPoint(direction.negate(), new Vector3()).sub(
                    Sb.supportPoint(direction, new Vector3()));
            final double lambda = direction.dot(sp) / direction.dot(direction) - envelope / direction.norm();
    ...
    private final void generate(final Result result) {
        ...
        Sa.supportFeature(contactNormal, faceA.insert());
        faceA.done();
        contactNormal.assignNegate();
        Sb.supportFeature(contactNormal, faceB.insert());
        faceB.done();
        ...
        final Iterator<Vector3> intersections = intersection.iterator();
        while (intersections.hasNext()) {
            // obtain next pair of intersection points
            final Vector3 p = intersections.next();
            final Vector3 q = intersections.next();

            // while points are still in contact space,
            // find their distance along the normal
            final double signedDistance = p.z - q.z - spb - spa;
    ...
    @Override
    public void getContactNormal(final Vector3 normal) {
